...
input_features = ['subcategory', 'segment', 'brand_name',
                  'discount_depth_rank', 'Promo_mechanic_en']
...
def train_promotion_prediction_model(input_data, input_features, cat_columns, model, learning_rate, max_depth,
    ...
    if remove_outliers:
        ...
        # set all +ve inc uplift % to 0
        outliers = input_data[input_data[output_features[0]] > 0]
        logger.info(
            "Setting all +ve {a} values to 0, {b} sample data points reset...".format(a=output_features[0],
                                                                                              b=outliers.shape[0]))
    ...
    if input_data[output_features].isnull().values.any():
        ...
        # HACK FOR NOW: Remove all null values
        input_data = input_data.dropna(axis=0, subset=output_features)
...
if __name__ == "__main__":
    ...
    input_data = pd.read_csv("C:\\Users\\hamzajuzer\\PycharmProjects\\prediction\\post_promotion_input_data.csv", sep=",",
                             encoding='latin1')
    ...
    if run_config == 'train' or run_config == 'train-predict':
        ...
        # save the train and filtered model as csv
        train_model.to_csv("post_train_model.csv", encoding='utf-8', index=False)
        filtered_model.to_csv("post_train_input.csv", encoding='utf-8', index=False)
        ...
        results_df.to_csv("post_results.csv", encoding='utf-8', index=False)
