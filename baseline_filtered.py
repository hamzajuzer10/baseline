...
def load_promo_from_bq(area, section, project_id):
    ...
    FROM `ETL.aggregate_promo_to_sku_summary_filtered`
...
if __name__ == "__main__":
    ...
    for i_sec in range(0, len(unique_sections)):
        ...
        if (i_sec == 0):
            pandas_gbq.to_gbq(results_df, 'baseline.baseline_filtered', project_id=project_id, if_exists=bl_table_config)
        else:
            pandas_gbq.to_gbq(results_df, 'baseline.baseline_filtered', project_id=project_id, if_exists='append')
